<template>
	<view class="prize_box">
		...
		<view class="prize_content" v-if="prize">
			...
			<text class="address" style="margin-top: 40px;font-size: 20px;">{{ prize.tips }}</text>
		...
		<view class="">
			<image style="width: 100%;" v-if="prize" :src="prize.bg_img" mode="aspectFit"></image>
			<text v-else>没有背景图片可显示</text>
		</view>
	</view>
</template>
...
<script setup>
	...
	const fetchPrizeData = async (param) => {
		try {
			const response = await getPrizeList({
				param
			})
