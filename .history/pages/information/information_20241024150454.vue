  ...
  <view class="container">
    <view class="information_box">
      <view class="info-item" v-for="(item, index) in infoArray" :key="index" :class="{ 'last-item': index === infoArray.length - 1 }">
        ...
        <input v-model="item.value" :placeholder="item.placeholder" type="text" @input="handleInput" :disabled="item.key === 'mobile'" /> <!-- 添加这个属性 -->
      </view>
    </view>
    <view class="footer">
...
onMounted(() => {
  uni.getStorage({
    key: 'userInfo',
    success: (res) => {
      const { mobile } = JSON.parse(res.data); // 使用解构赋值获取mobile属性
