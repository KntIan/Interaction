  ...
  <view class="container">
    ...
    <view v-if="showToast" class="custom-toast">
      ...
      <view class="toast-content">
        ...
        <button class="submitInterestedProduct">提交</button>
      </view>
    </view>
...
const onRadioChange = (value) => {
  ...
  // 更新 checked 的状态
  checkedValues.value = [value]; // 在这里更新选中的 checked 状态为当前值
...
const submitProduct = async () => {
  try {
    // 调用 API 提交选中的值
    const response = await apiSubmitInterestedProduct(selectedValue.value);
    console.log('提交结果:', response);

    // 可以进行其他处理，例如弹出提示或进行状态更新
    if (response.success) {
      console.log('提交成功！');
      // 逻辑处理，比如关闭弹窗或清空选择等
    } else {
