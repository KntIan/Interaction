...
const changeBackgroundImage = (item) => {
  ...
  // 通过查找索引并替换整个项目来保持响应式
  const index = radioOptions.value.findIndex(opt => opt.value === item.value);
  if (index !== -1) {
    radioOptions.value[index] = { ...item, bgImage: newBgImage }; // 创建一个新的对象
  }

  console.log('切换背景图片:', item.bgImage);
  console.log('切换背景图片:', radioOptions.value);
};
const closeToast = () => {
