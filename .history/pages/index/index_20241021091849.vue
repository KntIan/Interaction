      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions">
            ...
            <view class="bg_img" @click="changeBackgroundImage(item)">
...
const radioOptions = computed(() => {
  return attendanceStore.logs.map(log => ({
    ...
    checked: false, // 根据需要设置是否默认选中
     bgImage: '@/static/images/defaultBackground.png'
  }));
});
const changeBackgroundImage = (item) => {
  // 切换背景图片逻辑，可以根据需求设置不同的图片
  if (item.bgImage === '@/static/images/defaultBackground.png') {
    item.bgImage = '@/static/images/anotherBackground.png'; // 切换到另一个背景
  } else {
    item.bgImage = '@/static/images/defaultBackground.png'; // 切换回默认背景
  }
};
const closeToast = () => {
