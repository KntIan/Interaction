  ...
  <view class="container">
    ...
    <view v-if="showToast" class="custom-toast">
      ...
      <view class="toast-content">
        <view class="logo_tit">
          <image style="width: 145px;height: 20px;" src="@/static/images/Frame.png" mode="scaleToFill" />
          <text style="font-size: 14px;color: #000000;">请选择感兴趣的产品</text>
        </view>
        <!-- <view style="max-height: 420px;overflow-y: auto;" class="product_list">
          <radio-group @change="onRadioChange">
            <label v-for="item in radioOptions" :key="item.value">

              <view class="radio-content">
                <image style="width: 135px;height: 135px;" :src="item.image" class="radio-image" mode="aspectFit" />
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions">
            <image style="width: 135px;height: 135px;" class="log-image" :src="item.image" mode="aspectFit" />
            <text class="product-name">{{ item.label }}</text>
            <label class="custom-checkbox">
              <input type="checkbox" :value="item.value" v-model="item.checked" class="checkbox-input" />
...
const radioOptions = computed(() => {
  return attendanceStore.logs.map(log => ({
    ...
    label: log.content, // 假设 log 里有 content 字段用于显示的文字
...
.checkbox {
  ...
  height: 20px; /* 设置圆形大小 */
  border-radius: 50%; /* 圆形 */
  border: 2px solid #333; /* 圆形边框 */
  margin-left: 10px; /* 与文本的间距 */
  transition: background-color 0.2s;
}

.checkbox-input:checked + .checkbox {
  background-color: #005cb9; /* 选中后背景颜色 */
}
.custom-checkbox {
  position: relative;
  display: flex;
