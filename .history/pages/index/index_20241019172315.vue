      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions">
            ...
            <image style="width: 24px; height: 24px; margin: auto; padding-bottom: 10px;" :src="item.icon" @click="changeIcon(item)" />
          </view>
        </view>
...
const radioOptions = ref([]); // 初始化为一个空数组

// 计算属性，用于构建 radioOptions
const refreshRadioOptions = () => {
  radioOptions.value = attendanceStore.logs.map(log => ({
    value: log.id, // 每个 log 的唯一 id
    label: log.title, // log 的标题
    image: log.thumb, // log 的缩略图
    icon: '/static/images/iconno.png' // 初始图标路径
  }));
};
// 触发对 radioOptions 的更新
const updateRadioOptions = () => {
  refreshRadioOptions(); // 更新 radioOptions
};
// 更换图标路径的方法
const changeIcon = (item) => {
  item.icon = item.icon === '/static/images/iconno.png'
    ? '/static/images/iconis.png'
    : '/static/images/iconno.png'; // 切换图标
  console.log('图标路径:', item.icon);
  updateRadioOptions(); // 更新 radioOptions 以触发重新渲染
};
const closeToast = () => {
