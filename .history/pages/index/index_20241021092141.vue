  ...
  <view class="container">
    ...
    <view v-if="showToast" class="custom-toast">
      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions" @click="changeBackgroundImage(item)">
            ...
            <view class="bg_img" :style="{ backgroundImage: `url(${item.bgImage})`, width: '135px', height: '135px', borderRadius: '20px' }"></view>
          </view>
        </view>
...
const radioOptions = computed(() => {
  return attendanceStore.logs.map(log => ({
    ...
    bgImage: '@/static/images/iconno.png'
  }));
});
const changeBackgroundImage = (item) => {
  // 切换背景图片逻辑，可以根据需求设置不同的图片
  if (item.bgImage === '@/static/images/iconno.png') {
    item.bgImage = '@/static/images/iconnois.png'; // 切换到另一个背景
  } else {
    item.bgImage = '@/static/images/iconno.png'; // 切换回默认背景
  }
};
const closeToast = () => {
...
.bg_img {
  ...
  margin: auto;
  padding-bottom: 10px;
  background: url(@/static/images/iconno.png) no-repeat;
}
</style>
