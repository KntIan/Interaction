...
import { punchIn, getRecordList, getUrlParameter, submitInterestedProduct, checkLoginAndRedirect } from '@/utils/api';
...
const submitProduct = async () => {
  try {
    ...
    if (productInfo.value.is_like === 1) {
      openToast();
    }
    if (response.code === 1) {
      uni.showToast({
...
onShow(async () => {
  ...
  if (param) {
    try {
      ...
      isCheckedIn.value = punchResponse.data.is_like;
      showToast.value = punchResponse.data.product;
