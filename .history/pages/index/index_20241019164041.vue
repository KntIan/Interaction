      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions">
            ...
            <text style="padding: 10px;" class="product-name">{{ item.label }}</text>
            <image style="width: 24px; height: 24px; margin: auto; padding-bottom: 10px;" :src="item.icon" mode="scaleToFill" @click="changeIcon(item)" />
          </view>
        </view>
...
const changeIcon = (item) => {
  // 更改图标的路径为新的路径
  item.icon = '@/static/images/iconis.png'; // 替换为 iconis.png 的路径
};
...
const radioOptions = computed(() => {
  return attendanceStore.logs.map(log => ({
    ...
    icon: '@/static/images/icon.png' // 假设每个 log 有一个默认的 icon 路径
  }));
});
const closeToast = () => {
