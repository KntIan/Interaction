  ...
  <view class="container">
    ...
    <view v-if="showToast = true" class="custom-toast">
      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item,index in radioOptions" :key="index">
            <image class="log-image" :src="item.image" />
            <text class="product-name">{{ item.label }}</text>
            <!-- <view style="margin: auto;" @click="changeIcon(index)">
              <image style="width: 24px; height: 24px;" :src="icons[index] || '/static/images/iconno.png'" mode="scaleToFill" />
            </view>
            <input style="padding-bottom: 10px;" type="radio" :value="item.value" v-model="selectedValue" @click="onRadioChange(item.value)" /> -->
            <view style="margin: auto;" @click="handleRadioChange(item.value)">
              <image style="width: 24px; height: 24px;" :src="icons[index] || '/static/images/iconno.png'" mode="scaleToFill" />
            </view>
          </view>
        </view>
...
const changeIcon = (index) => {
  ...
  icons.value[index] = (icons.value[index] === '/static/images/iconno.png')
    ...
    : '/static/images/iconno.png';
  console.log(icons.value); // 输出当前 icons 状态
};
...
const handleRadioChange = (value, index) => {
  selectedValue.value = value; // 设置 v-model 的值
  onRadioChange(value); // 调用 onRadioChange 函数
  changeIcon(index)
};
...
onShow(async () => {
  ...
  if (param) {
    try {
      ...
      // 之后同步 icons 的长度
      icons.value = new Array(attendanceStore.logs.length).fill('/static/images/iconno.png');
