...
const selectedIds = ref([]); // 定义用来存储选中的 ID 数组
...
const onRadioChange = (value) => {
  ...
  console.log('当前选中的 ID 数组:', selectedIds.value);
...
const submitProduct = async () => {
  try {
    ...
    // 调用 API 提交选中的值，将 selectedIds 数组传递给后端
    const response = await submitInterestedProduct({ param, selectedIds: selectedIds.value });
