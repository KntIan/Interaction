  ...
  <view class="container">
    ...
    <view v-if="showToast" class="custom-toast">
      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions" :key="item.value" @click="changeBackgroundImage(item)">
            ...
            <view class="bg_img" :style="{ backgroundImage: `url(${item.bgImage})` }"></view>
          </view>
        </view>
...
const radioOptions = reactive(attendanceStore.logs.map(log => ({
  value: log.id,
  label: log.title,
  image: log.thumb,
  checked: false,
  bgImage: '/h5/static/images/iconno.png'
})));
console.log(radioOptions.logs);
const changeBackgroundImage = (item) => {
  const newBgImage = item.bgImage === '/h5/static/images/iconno.png'
    ? '/h5/static/images/iconnois.png'
    : '/h5/static/images/iconno.png';

  // 创建一个新对象以强制触发更新
  Object.assign(item, { bgImage: newBgImage });

  console.log('切换背景图片:', item.bgImage);
  console.log('切换背景图片:', radioOptions.value); // 注意：需要获取它的值
};
const closeToast = () => {
