      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions">
            ...
            <image style="width: 24px; height: 24px; margin: auto; padding-bottom: 10px;" :src="item.isChecked ? '/static/images/iconis.png' : '/static/images/iconno.png'" @click="changeIcon(item)" />
          </view>
        </view>
...
const isChange = ref(true); // 初始化打卡状态
...
const changeIcon = (item) => {
  ...
  const updatedOptions = radioOptions.value.map(opt => {
    if (opt.value === item.value) {
      return { ...opt, isChecked: !opt.isChecked }; // 复制对象，且切换状态
    }
    return opt; // 返回其他未改变的对象
  });

  // 更新 radioOptions 的状态
  radioOptions.value = updatedOptions; // 赋值更新
  console.log('点击了选项:', item.isChecked);
}
...
const radioOptions = computed(() => {
  return attendanceStore.logs.map(log => ({
    ...
    icon: '/static/images/iconno.png',
    isChecked: false // 新增属性，用于管理每个选项的状态
  }));
});
const closeToast = () => {
