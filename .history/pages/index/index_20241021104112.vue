      ...
      <view class="toast-content">
        ...
        <view class="logs_box">
          <view class="item" v-for="item in radioOptions" :key="item.value">
            ...
            <input type="radio" :value="item.value" v-model="selectedValue" @click="onRadioChange(item.value)" :class="{ 'checked': selectedValue === item.value }" />
          </view>
        </view>
...
import { punchIn, getRecordList, getUrlParameter, submitInterestedProduct } from '@/utils/api'; // 引入指定的接口方法
...
const selectedValue = ref(''); // 用于存储选择的 radio 值
const ids = ref([]); // 定义用来存储选中的 ID 数组
// 存储 radioOptions 的 checked 状态
const checkedValues = ref([]);
// 计算属性，用于构建 radioOptions
const radioOptions = computed(() => {
  return attendanceStore.logs.map(log => ({
    value: String(log.id), // 转换为字符串 
    label: log.title, // 假设 log 里有 content 字段用于显示的文字
    image: log.thumb, // 假设 log 里有 thumb 字段用于显示的图像
    checked: checkedValues.value.includes(String(log.id)) // 检查当前项是否被选中
  }));
});
const closeToast = () => {
...
const onRadioChange = (value) => {
  const numericValue = Number(value); // 转换为数字
  ...
  selectedValue.value = numericValue; // 更新选中的值
  console.log('当前选中的 ID 数组:', ids.value);
...
const submitProduct = async () => {
  try {
    ...
    // 将 ids 数组转换为以逗号分隔的字符串
    const idsString = ids.value.join(',');

    // 调用 API 提交选中的值，将 ids 数组传递给后端
    const response = await submitInterestedProduct({ param, ids: ids.value });
...
input[type="radio"].checked {
  outline: none; /* 去掉默认的外轮廓 */
  border: 2px solid #4caf50 !important; /* 设置边框颜色 */
  background-color: #e0f7e9 !important; /* 设置背景颜色 */
  border-radius: 50%; /* 确保是圆形 */
}

/* 还可以为外层添加位置来突出显示选中状态 */
.item .checked + .log-image {
  border: 2px solid #4caf50 !important; /* 选中时可以给图像添加边框 */
}
</style>
