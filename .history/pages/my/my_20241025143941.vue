<template>
  <view class="my_box" v-if="info">
    ...
    <view class="info prize" @click="handlePrizeClick">
    ...
    <view v-if="isModalVisible" class="modal">
      <view class="close-btn" @click="hidePrizeModal">✖</view> <!-- 关闭按钮 -->
    ...
    <!-- 提示信息 -->
    <view v-if="message" class="tip">{{ message }}</view>
  </view>
</template>
...
const deletePrize = async (id) => {
  try {
    ...
    message.value = "删除记录成功"; // 提示用户删除成功
    // 可选：设置消息在一段时间后消失
    setTimeout(() => { message.value = null; }, 2000);
  } catch (error) {
    console.error('删除记录失败:', error)
    message.value = "删除记录失败"; // 提示用户删除失败
    setTimeout(() => { message.value = null; }, 2000);
  }
}
...
const handlePrizeClick = () => {
  if (prizeRecords.value.length === 0) {
    message.value = "还没有可领取的奖品"; // 提示用户没有可领取的奖品
    setTimeout(() => { message.value = null; }, 2000); // 提示显示 2 秒后消失
  } else {
    showPrizeModal(); // 已有奖品记录，打开弹窗
  }
}
