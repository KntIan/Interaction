      ...
      <view class="modal-content">
        <view class="modal-body">
          <view class="item_" v-for="(item) in prizeRecords" :key="item.value">
            ...
            <!-- <view style="padding-bottom: 10px;" class="award_title" @click="deletePrize(item.id,index)"> -->
            <view style="padding-bottom: 10px;" class="award_title" @click="handleRadioChange(item.id,index)">
...
const updateIcons = () => {
  icons.value = prizeRecords.value.map(() => '/static/images/iconno.png');
};
...
const deletePrize = async (id) => {
  try {
    await deletePrizeRecord({ id: id }); // 调用后端接口删除奖品记录
