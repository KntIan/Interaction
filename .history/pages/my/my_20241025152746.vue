...
import TnPopup from "@/uni_modules/tuniaoui-vue3/components/TnPopup/TnPopup.vue";
...
const deletePrize = async (id) => {
  try {
    ...
    // 使用uni.showToast进行提示，自动消失
    uni.showToast({
      title: '删除记录成功',
      icon: 'success',
      duration: 2000 // 提示显示 2 秒后自动消失
    });
  } catch (error) {
...
const handlePrizeClick = () => {
  if (prizeRecords.value.length === 0) {
    showPopup.value = true // 弹框提示用户还没有奖品记录
    // uni.showToast({
    //   title: '还没有可领取的奖品',
    //   icon: 'error',
    //   duration: 2000 // 提示显示 2 秒后自动消失
