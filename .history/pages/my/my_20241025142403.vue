        ...
        <view class="modal-body" :style="{ columnCount: columnCount }">
          <view class="item_" v-for="(item) in prizeRecords" :key="item.id">
            ...
            <view style="padding-bottom: 10px;" class="award_title">
              <radio :value="String(item.id)" :checked="selectedPrizeId.includes(String(item.id))" @change="() => togglePrizeSelection(String(item.id))" />
            </view>
          </view>
        </view>
        <view class="footer">
...
const selectedPrizeId = ref([]); // 初始化为空数组
...
const togglePrizeSelection = (id) => {
  if (selectedPrizeId.value.includes(id)) {
    selectedPrizeId.value = selectedPrizeId.value.filter(item => item !== id); // 移除已选中的 ID
  } else {
    selectedPrizeId.value.push(id); // 添加新的 ID
  }
};
const deletePrize = async () => {
  if (!selectedPrizeId.value) {
    console.error('未选择任何奖品进行删除');
    return;
  }

  try {
    await deletePrizeRecord({ id: String(selectedPrizeId.value) }); // 确保传入字符串类型的 ID
    prizeRecords.value = prizeRecords.value.filter(item => item.id !== selectedPrizeId.value)
    selectedPrizeId.value = null; // 清空选中的奖品 ID
...
const hidePrizeModal = () => {
  ...
  selectedPrizeId.value = null; // 关闭弹窗时清空选中状态
}
...
.footer button {
  ...
  box-shadow: 0 8px 10px rgba(2, 118, 255, 0.2);
  color: white;
  border: none;
  border-radius: 24px;
  cursor: pointer;
