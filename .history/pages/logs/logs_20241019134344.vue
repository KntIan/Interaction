  ...
  <view class="box">
    ...
    <view class="logs_box">
      <view class="item" v-for="(log, index) in logs" :key="index">
        ...
        <view class="status" :class="{ 'checked-in': log.is_point === 1, 'not-checked-in': log.is_point === 0 }">
          <image v-if="log.is_point === 1" style="width: 16px; height: 16px;" src=" @/static/images/Fram1e.png " mode="aspectFill" />
          <text>{{ log.is_point? }}</text>
        </view>
      </view>
    </view>
  </view>
