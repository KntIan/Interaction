<template>
  <view class="box">
    <view class="title">打卡记录</view>
    <view class="logs_box">
      <view class="item" v-for="(log, index) in attendanceStore.logs" :key="index">
        <image style="height: 165px;" class="log-image" :src="log.thumb" mode="" />
        <view class="product-name">{{ log.title }}</view>
        <view class="status" :class="{ 'checked-in': log.is_point, 'not-checked-in': !log.is_point }">
          <image v-if="log.is_point" style="width: 16px; height: 16px;" src="@/static/images/Fram1e.png" mode="aspectFill" />
          <text>{{ log.is_point ? '已打卡' : '未打卡' }}</text>
        </view>
      </view>
    </view>
  </view>
</template>

<script setup>
import {
  onMounted
} from 'vue';
import {
  onShow
} from '@dcloudio/uni-app';
import {
  getRecordList,
  getUrlParameter
} from '@/utils/api';
import {
  useAttendanceStore
} from '@/store/store';

const attendanceStore = useAttendanceStore(); // 使用 Pinia store
const param = getUrlParameter("param") || uni.getStorageSync("param");

const fetchLogs = async (param) => {
  try {
    const response = await getRecordList({
      param
    });
    return response.data || [];
  } catch (error) {
    console.error('获取打卡记录失败:', error);
    throw new Error('获取数据出错');
  }
};

onShow(async () => {
  if (param) {
    try {
      uni.setStorageSync("param", param);
      console.log("param存储成功");

      const logs = await fetchLogs(param);
      attendanceStore.setLogs(logs); // 存储到 Pinia 中
      console.log('获取打卡记录成功:', attendanceStore.logs);
    } catch (error) {
      uni.showToast({
        title: error.message || '获取打卡记录失败',
        icon: 'none',
      });
    }
  } else {
    console.warn('未找到param参数');
    uni.showToast({
      title: '缺少param参数',
      icon: 'none',
    });
  }
});
</script>

<style scoped>
.box {
  background: #f7f8fa;
  min-height: calc(100vh - env(safe-area-inset-top)); /* 减去安全区域的高度 */
  padding-top: env(safe-area-inset-top); /* 添加顶部安全区域的padding */
}

.title {
  font-weight: 500;
  font-size: 16px;
  color: #333;
  text-align: center;
  padding: 12px 0;
  position: fixed;
  left: 0;
  right: 0;
  z-index: 99;
  background: #f7f8fa;
}

.logs_box {
  column-count: 2;
  /* 设置列数为2，形成瀑布流布局 */
  column-gap: 10px;
  /* 列之间的间距 */
  padding: 10px;
  padding-top: 55px;
}

.item {
  display: inline-block;
  /* 使用 inline-block 使得瀑布流效果更流畅 */
  break-inside: avoid;
  /* 防止在列内断裂 */
  width: 100%;
  /* 宽度设置为100%以适应不同列 */
  margin-bottom: 10px;
  border-radius: 20px;
  transition: transform 0.3s;
  box-shadow: 0 0 20px rgba(0, 92, 185, 0.1);
  background: #fff;
}

.item:hover {
  transform: scale(1.02);
}

.log-image {
  width: 100%;
  height: 100%;
  object-fit: cover;
  border-radius: 20px;
}

.product-name {
  overflow: hidden;
  text-overflow: ellipsis;
  margin: 10px;
  font-weight: bold;
  color: #333;
}

.status {
  width: 120px;
  height: 36px;
  line-height: 36px;
  text-align: center;
  color: white;
  border-radius: 100px;
  display: flex;
  justify-content: center;
  align-items: center;
  margin: auto;
  margin-bottom: 10px;
  margin-top: 10px;
}

.checked-in {
  background: linear-gradient(90deg, #005cb9 0%, #56b7e6 100%);
}

.not-checked-in {
  background-color: #b3b3b3;
}
</style>