...
const punchLogs = ref({ list: [] }); // 初始化为包含 list 的对象
const completedCount = ref(0);
const pendingCount = ref(0);

// 更新打卡统计
const updateCounts = () => {
  const total = punchLogs.value.list.length;
  completedCount.value = punchLogs.value.list.filter(
    (log) => log.isPunched
  ).length;
  pendingCount.value = total - completedCount.value;
};
...
onMounted(fetchPunchLogs);

onShow(fetchPunchLogs);

// 监听事件
onBeforeUnmount(() => {
  eventBus.off("punchData");
});

// 监视 punchLogs 的变化以更新 counts
watch(punchLogs, updateCounts);
</script>
...
.locations {
  ...
  box-shadow: 0px 0px 10px rgba(78, 136, 255, 0.1);
  border-radius: 15px;
  padding: 27px 19px;
  margin-bottom: 20px;
}
