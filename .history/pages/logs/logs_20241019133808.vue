  ...
  <view class="box">
    ...
    <view class="logs_box">
      <view class="item" v-for="(log, index) in logs" :key="index">
        <image class="log-image" :src="log.image" mode="aspectFit" />
        <text class="product-name">{{ log.productName }}</text>
        <view class="status" :class="{ 'checked-in': log.checkInStatus === '已打卡', 'not-checked-in': log.checkInStatus === '未打卡' }">
...
onMounted(async () => {
  if (param) {
    try {
      ...
      console.log('获取打卡记录成功:', response.data);
    } catch (error) {
