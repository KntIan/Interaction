<template>
  <view class="logs_box">
    <view ref="root" class="item" v-for="(log, index) in logs" :key="index">
...
<script setup>
import { ref, watch, nextTick ,on} from 'vue';
const root = ref(null); // 定义 root 的引用
const logs = ref([
  {
...
const someProperty = ref('initial value'); // 你可以动态改变这个属性来触发 watch

watch(someProperty, () => {
  nextTick(() => {
    const el = root.value;
    console.log(el); // 检查元素是否存在
    if (!el) {
      console.error('Element not found!');
      return; // 如果元素不存在，提前返回，避免后续的错误
    }
