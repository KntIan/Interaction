...
const httpRequest = (method) => async (url, data) => {
  if (!url) {
    return Promise.reject(new Error('请求的 URL 不能为空'))
  }
  return new Promise((resolve, reject) => {
    uni.request({
      url: `http://daka.nanjingyunpeng.cn/api${url}`, // 确保 base URL 和 url 参数正确
...
export const getUrlParameter = (paramName) => {
  const href = uni.getCurrentPage().route // 获取当前页面路由
  const paramMatch = href.match(new RegExp(`[?&]${paramName}=([^&#]*)`))
  return paramMatch ? paramMatch[1] : null
}
