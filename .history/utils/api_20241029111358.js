...
// 	baseURL: 'http://daka.nanjingyunpeng.cn/api', // 替换为您的API基础URL
...
const httpRequest = (method) => async (url, data) => {
  return new Promise((resolve, reject) => {
    uni.request({
      url: `http://daka.nanjingyunpeng.cn/api${url}`, // API基础URL需加在这里
...
export const checkLoginAndRedirect = () => {
  ...
  if (token) {
    ...
    // 用户未登录，跳转到登录页
    uni.navigateTo({
      url: '/pages/login/login?param=uc3bZQPTOMYne9C1',
    })
    console.log('未登录，跳转至登录页')
  }
}
...
export const login = async (credentials) => {
  ...
  // 存储参数 uc3bZQPTOMYne9C1
  const param = 'uc3bZQPTOMYne9C1'
